    ...
    if current_user.is_authenticated: 
        ...
        if request.method == "POST":
            for item in request.POST:
                ...
                try:
                    variation = Variation.objects.get(product=product,variaton_category__iexact=key, variaton_value__iexact=value)
...
def Carts(request,total=0,quantity=0,tax=0,grand_total=0,cart_items=None):
    try:
        if request.user.is_authenticated:
